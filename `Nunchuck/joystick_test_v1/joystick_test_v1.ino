...
byte joystickX;     // Min: 25  Resting: 128       Max: 231
byte joystickY;     // Min: 28  Resting: 129/130   Max: 226
byte zButton;
byte cButton;

int factorDeadzone(byte val) {
  if (128 - deadzone < val && val < 128 + deadzone) { // if nunchuck is within deadzone
    return 0;
  } else if (val < 128 - deadzone) { // if nunchuck is below deadzone (negative x/y)
    return -1;
  } else if (128 + deadzone < val) { // if nunchuck is above deadzone (positive x/y)
    return 1;
  }
}
...
void loop() {
  ...
  int xSign = factorDeadzone(joystickX);  // int value between -1 and 1. For now, there is no steering control (full turning or nothing)
  int ySign = factorDeadzone(joystickY);  // int value between -1 and 1. For now, there is no speed control (full throttle or nothing)
