...
// both x & y pins must be PWM capable
const int xPin = 5;
const int yPin = 6;
...
void loop() {
  ...
  directionY = getDirection(nunchuck.getJoyY());
  int escMin = 920;
  int escMid = 1460;
  int escMax = 2000;
  if (directionY == 1) {
    esc.writeMicroseconds(map(nunchuck.getJoyY(), joyMiddle + deadzone, joyMax, escMid, escMax));
    Serial.println(map(nunchuck.getJoyY(), joyMiddle + deadzone, joyMax, escMid, escMax));
  } 
  else if (directionY == -1) {
    esc.writeMicroseconds(map(nunchuck.getJoyY(), joyMiddle - deadzone, joyMin, escMid, escMin));
    Serial.println(map(nunchuck.getJoyY(), joyMiddle - deadzone, joyMin, escMid, escMin));
  } 
